...
public class ArrayUtils {
    ...
    public static void sortingByChoose(int[] intArray, boolean ascending) {
        for (int referValue = 0; referValue < intArray.length - 1; referValue++) {
            int middle = intArray[referValue];
            int min = 0;
            for (int module = referValue + 1;
                 module <= intArray.length - 1;
                 module++) {
                boolean temp = true;
                if (ascending) {
                    temp = middle > intArray[module];
                }
                else {
                    temp = middle < intArray[module];
                }
                if (temp) {
                    middle = intArray[module];
                    min = module;
                }
            }
            ...
            if (min != 0) {
                int f = intArray[min];
                intArray[min] = intArray[referValue];
                intArray[referValue] = f;
            }
        }
    }
